...
## Implemented (2026-06 fork, ultra-performance pass — WebSocket push + shared store — tested, iteration_5.json, frontend 100% / backend 12/12)
- User brief: production-grade performance/stability, "instant, fluid, trading-terminal feel". Choices: 1a+1b (apply principles to real app AND build a real WebSocket push feed), 2a (~60s upstream cadence, rate-limit safe), 3a (perf/stability only — no new features this round)
- Backend (server.py): ONE server-side poller (~60s) via `fetch_market_snapshot()` + `market_poller()`/startup event fans snapshots to all clients over `@app.websocket('/api/ws/prices')` (per-connection asyncio.Queue via `PriceHub`, snapshot-on-connect + ping/pong heartbeat, bounded queue = no backlog). Verified working through the ingress (wss://). REST `/api/market-prices` kept as graceful fallback. Cache-Control added to market-prices/chart/search (note: ingress overrides REST cache-control to no-store — WS is primary). Partial-failure fix: snapshot now merges by symbol against last-known so a CoinGecko 429 no longer evicts BTC/ETH/SOL
- Frontend shared store (`lib/marketStore.js`): replaced 3 duplicate 60s pollers (MarketTicker, MarketBoard, nav LiveMini) with ONE WebSocket connection via `useMarketData()`/`useMarketItem()` (useSyncExternalStore). Background-tab pause (idle grace), reconnect w/ exponential backoff+jitter, 25s heartbeat + 90s stale watchdog, ref-counted lifecycle (survives route changes), REST fallback after 3 WS failures + periodic WS retry, per-symbol ref reuse so nav BTC mini doesn't re-render on unrelated updates
- Chart perf: `lib/chartCache.js` bounded LRU (TTL 5m + size 60); MarketChart uses fetch + AbortController (true request cancellation) + cache; tooltip rAF-throttled with cached rect + O(1) nearest-index (no per-move getBoundingClientRect)
- Canvas perf (DataSkyline, GlobalNetwork): ResizeObserver fit, visibilitychange pause (stops rAF on hidden tab), prefers-reduced-motion static frame, adaptive DPR + detail by device tier (`lib/perf.js` getPerfTier via deviceMemory/hardwareConcurrency), coarse-pointer disables mouse parallax, DataSkyline gradient cached per-fit (was allocating one gradient per bar per frame)
- Code-splitting: all 7 pages React.lazy + Suspense fallback in App.js (smaller initial bundle)
- Dev-only PerfMonitor HUD (FPS/frame/mem/feed/tier) gated by `?perf=1` or localStorage `oki-perf=1` — absent for normal users
- Verified: WS live end-to-end (snapshot=11 + ping/pong on wss://), ticker/board/nav populate ~2s, all 7 routes render, theme toggle intact, perf HUD gating correct, ZERO console errors (the single ERR_ABORTED is intentional fetch cancellation)
...
## Implemented (2026-08-12, corporate email rollout)
- contact@okiinc.global added sitewide: Contact page (mailto button beside phone + At-a-Glance card row), Footer (under phone), About HQ section, PDF deck contact line, and the inquiry email template footer
